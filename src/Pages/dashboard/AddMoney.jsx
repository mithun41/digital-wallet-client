...
import { ArrowLeft, Smartphone, CreditCard, Building2, Shield, Info } from 'lucide-react';
...
const AddMoney = () => {
  ...
  const handleAddMoney = () => {
    ...
    setTimeout(() => {
      ...
      alert(`Successfully initiated add money of ৳${amount} via ${methods.find(m => m.id === selectedMethod)?.name}`);
    }, 2000);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-50 to-purple-50">
      ...
      <div className="p-4 mt-6">
        <div className="bg-white rounded-xl p-6 shadow-lg border border-pink-100">
          <label className="block text-gray-700 font-medium mb-3">Enter Amount</label>
          <div className="relative">
            <span className="absolute left-4 top-1/2 transform -translate-y-1/2 text-pink-600 font-bold text-xl">৳</span>
            <input
              type="number"
              value={amount}
        ...
        <div className="space-y-3">
          {methods.map((method) => (
            <div
              ...
              className={`bg-white rounded-xl p-4 shadow-lg border-2 cursor-pointer transition-all duration-200 hover:shadow-xl ${
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className={`p-3 rounded-xl ${
                    selectedMethod === method.id ? 'bg-pink-100 text-pink-600' : 'bg-gray-100 text-gray-600'
                  }`}>
                    {method.icon}
                  </div>
                  <div>
                ...
                <div className="text-right">
                  <p className="text-sm font-medium text-gray-600">Fee: {method.fee}</p>
          ...
          <div className="flex gap-3">
            ...
            <div>
              <p className="text-sm text-blue-800">
                <span className="font-medium">Security Notice:</span> Your transaction is protected with bank-level security. 
                Money will be added instantly to your bKash wallet.
              </p>
            </div>
          </div>
        </div>
        ...
        {selectedMethod && amount && (
          <div className="text-center mt-2">
            <p className="text-sm text-gray-500">
              ...
              Total: ৳{selectedMethod === 'card' ? (parseFloat(amount || 0) * 1.0185).toFixed(2) : amount || 0}
            </p>
          </div>
        )}
      </div>
